...
// Local-mode equivalent of src/components/apps/AppsListView, but the row
// links point at `/workspace/app/?id=…` (single static page that reads the
// id from search params) instead of the SaaS app's `/agents/[id]/apps/[id]`
// dynamic route — which the static export can't generate at build time.
  ...
  if (state !== ConnectionState.CONNECTED && state !== ConnectionState.CONNECTING) {
    return (
      <p className="rounded-md border border-border bg-bg-subtle p-4 text-sm text-fg-muted">
        Not connected to the gateway. Apps live on your gateway and are only reachable while
        connected.
      </p>
    );
  }
  ...
  if (apps.length === 0) {
    return (
      <p className="rounded-md border border-dashed border-border p-6 text-sm text-fg-muted">
        No apps yet. Ask the agent to build one.
      </p>
    );
  }
